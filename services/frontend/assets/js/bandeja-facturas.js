(function initBandeja() {
  ...
  async function syncInbox(showToast = true) {
    ...
    if (syncMeta && result?.synced_at) {
      syncMeta.textContent = `Ultima sync: ${new Date(result.synced_at).toLocaleString()} - Auto-sync cada 5 min`;
    }

    if (showToast) {
      const errors = Array.isArray(result?.errors) ? result.errors.filter(Boolean) : [];
      const message = `Sync completada. Nuevas: ${result?.created || 0}. Actualizadas: ${result?.updated || 0}. Procesadas: ${result?.processed_messages || 0}.`;
