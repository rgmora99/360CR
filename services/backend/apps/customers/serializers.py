...
class CustomerSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        attrs = clean_party_identity(attrs, instance=self.instance, type_field_name="customer_type", party_label="cliente")
        ...
        if self.instance and ("code" in attrs or "organization" in attrs):
            ...
            if not code:
                raise serializers.ValidationError({"code": "El codigo del cliente no puede quedar vacio."})
            attrs["code"] = code

        if self.instance and organization and code:
            queryset = Customer.objects.filter(organization=organization, code=code)
            queryset = queryset.exclude(pk=self.instance.pk)
            if queryset.exists():
                raise serializers.ValidationError({"code": "Ya existe un cliente con ese codigo en esta organizacion."})
